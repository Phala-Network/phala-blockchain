...
use sp_consensus_grandpa::{AuthorityId, AuthorityWeight, ConsensusLog, GRANDPA_ENGINE_ID, ScheduledChange, SetId};
use sp_core::H256;
use sp_runtime::traits::{Block as BlockT, Header, NumberFor};
use sp_runtime::EncodedJustification;

pub use types::AuthoritySet;
    ...
    NumberFor<T::Block>: AsPrimitive<usize>,
{
    ...
    pub fn submit_finalized_headers(
        ...
        match self.tracked_bridges.get_mut(&bridge_id) {
            Some(bridge_info) => {
                if let Some(scheduled_change) = Self::find_scheduled_change(&header) {
