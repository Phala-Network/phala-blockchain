...
pub struct Phactory<Platform> {
    ...
    // FIXME: maybe block number is better ?
...
impl<Platform: pal::Platform> Phactory<Platform> {
    ...
    // TODO: george should refactor the save/load for runtime state 
    // the underlying storage makes us have no need for the seal key 
...
impl<Platform: pal::Platform + Serialize + DeserializeOwned> Phactory<Platform> {
    ...
    pub fn restore_from_checkpoint(
        ...
        info!("Succeeded to load checkpoint with the key {:?}", CHECKPOINT_RUNTIME_DUMP_STORAGE_KEY);
