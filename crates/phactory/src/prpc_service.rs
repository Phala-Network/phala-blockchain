    ...
    fn get_worker_key(
        ...
        if !dev_mode {
            let runtime_info = phactory
                ...
                .as_ref()
                .ok_or_else(|| from_display("Runtime not initialized"))?;
            let my_runtime_hash = sp_core::hashing::blake2_256(&runtime_info.encoded_runtime_info);
            let runtime_state = phactory.runtime_state()?;
            let my_runtime_timestamp = chain_state::get_pruntime_timestamp(
                &runtime_state.chain_storage,
            ...
            let req_runtime_timestamp = chain_state::get_pruntime_timestamp(
                &runtime_state.chain_storage,
                &req_runtime_hash,
            )
