...
impl<Platform: pal::Platform + Serialize + DeserializeOwned> PhactoryApi for RpcService<Platform> {
    ...
    async fn handover_start(
        ...
        // 2. verify challenge block height and report timestamp
        ...
        if !dev_mode && in_sgx {
            ...
            sgx_api_lite::verify(recv_local_report)
                .map_err(|_| from_display("LA verify failed"))?;
            let remote_mr = report_to_mr(&recv_local_report);
            if Some(remote_mr) == phactory.allow_handover_to {
                info!("Allowed handover to {}", hex_fmt::HexFmt(&remote_mr));
                allowed_mr = true;
            }
        } else {
        ...
        if !dev_mode && in_sgx && !allowed_mr {
            // 4. verify client RA report to ensure it's in sgx
            ...
            info!("Skip client RA report and pRuntime timestamp check");
        };
        ...
        let attestation = if !dev_mode && in_sgx && !allowed_mr {
            ...
            info!("Omit RA report in workerkey response");
            None
        };
...
fn my_measurement() -> Result<Vec<u8>, RpcError> {
    ...
    Ok(report_to_mr(&my_la_report))
}

fn report_to_mr(report: &sgx_api_lite::Report) -> Vec<u8> {
    IasFields {
