...
impl<Platform: pal::Platform + Serialize + DeserializeOwned> PhactoryApi for RpcService<Platform> {
        ...
        let attestation = if !dev_mode && in_sgx {
            ...
            let attn_to_validate =
                Option::<AttestationReport>::decode(&mut &raw_attestation.encoded_report[..])
                    .map_err(|_| from_display("Decode attestation report failed"))?;
        ...
        if !dev_mode && in_sgx {
            let recv_local_report = unsafe {
                sgx_api_lite::decode(&challenge_handler.sgx_local_report)
                    .map_err(|_| from_display("Invalid LA report from client"))?
            };
            sgx_api_lite::verify(recv_local_report)
                .map_err(|_| from_display("No remote handover"))?;
        } else {
            info!("Skip LA report check in dev mode");
        }
        ...
        if !dev_mode && in_sgx {
            ...
            let my_attn_report =
                Option::<AttestationReport>::decode(&mut &my_attn.encoded_report[..])
                    .map_err(|_| from_display("Decode my attestation report failed"))?;
            let my_attn_report =
                my_attn_report.ok_or_else(|| from_display("My attestation report not found"))?;
    ...
    async fn handover_accept_challenge(
        ...
        let attestation = if !dev_mode {
            ...
            info!("Omit RA report for dev mode challenge");
            None
        };
    ...
    async fn handover_receive(&mut self, request: pb::HandoverWorkerKey) -> RpcResult<()> {
        ...
        if !dev_mode {
            ...
            info!("Skip RA report check for dev mode key");
        }
