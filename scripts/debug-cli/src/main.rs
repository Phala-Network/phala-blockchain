...
fn main() {
    ...
    match cli {
        ...
        Cli::DecodePruntimeInfo { hex_data, print_field } => {
            let data = hex::decode(hex_data)
                .expect("Failed to parse hex_data");
        ...
        Cli::DecodeWorkerSnapshot { hex_data } => {
            let data = hex::decode(hex_data)
                .expect("Failed to parse hex_data");
            let snapshot = phala_types::pruntime::OnlineWorkerSnapshot::<u32, u128>
        ...
        Cli::DecodeBhwe { b64_data } => {
            ...
            let data = base64::decode(&b64_data)
                .expect("Failed to decode b64_data");
            let snapshot = phala_types::pruntime::BlockHeaderWithEvents::<u32, BlakeTwo256, u128>
                ::decode(&mut data.as_slice());
