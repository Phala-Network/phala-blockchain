...
echo "----------- starting phost -------------"
cd /root/phala-blockchain && ./target/release/phost --no-write-back --mnemonic "then prefer table fatal bus portion refuse chunk attend real horror cat" --pruntime-endpoint "http://localhost:8000" --substrate-ws-endpoint "ws://localhost:9944" > /root/phost.out 2>&1 &
