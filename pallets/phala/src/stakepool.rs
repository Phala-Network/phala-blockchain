...
pub mod pallet {
		...
		T: pallet_uniques::Config<CollectionId = CollectionId, ItemId = NftId>,
	{
		...
		pub fn claim_owner_shares(
			...
			let nft_id = basepool::Pallet::<T>::mint_nft(&pool_info.basepool, target.clone(), shares.clone(), rewards)?;
		...
		pub fn vault_check_and_maybe_force_withdraw(
			...
			for pid in vault.invest_pools.iter() {
				...
				for withdraw in withdraw_vec {
					let nft =
						basepool::Pallet::<T>::get_nft_attr(&stake_pool.basepool, withdraw.nft_id)?;
		...
		pub fn vault_withdraw(
			...
			let mut nft = basepool::Pallet::<T>::get_nft_attr(&pool_info.basepool, nft_id)?;
			let in_queue_shares = match pool_info
				.basepool
				.withdraw_queue
				.iter()
				.find(|&withdraw| withdraw.user == vault_info.user_id.clone())
			{
				Some(withdraw) => {
					let withdraw_nft =
			...
			basepool::Pallet::<T>::set_nft_attr(&pool_info.basepool, nft_id, &nft)
		...
		fn try_process_withdraw_queue(
			...
			while basepool::is_nondust_balance(pool_info.free_stake) {
				if let Some(withdraw) = pool_info.withdraw_queue.front().cloned() {
					...
					basepool::Pallet::<T>::set_nft_attr(&pool_info, withdraw.nft_id, &withdraw_nft)
	...
	mod test {
		...
		// Pallets
		use crate::mock::{
			Balances, PhalaMining, PhalaRegistry, PhalaStakePool, System, Timestamp, PhalaBasePool,
		};
		...
		fn test_merge_or_init_nft() {
			new_test_ext().execute_with(|| {
				...
				assert_ok!(PhalaBasePool::merge_or_init_nft_for_staker(&pool_info.basepool, 1));
				let nftid_arr: Vec<NftId> =
					pallet_rmrk_core::Nfts::<Test>::iter_key_prefix(0).collect();
				assert_eq!(nftid_arr.len(), 1);
				let nft_attr = PhalaBasePool::get_nft_attr(&pool_info.basepool, nftid_arr[0]).unwrap();
				assert_eq!(nft_attr.shares, 3000 * DOLLARS);
				assert_eq!(nft_attr.stakes, 3000 * DOLLARS);
				assert_ok!(PhalaBasePool::merge_or_init_nft_for_staker(&pool_info.basepool, 2));
		...
		fn test_set_nft_attr() {
			new_test_ext().execute_with(|| {
				...
				assert_ok!(PhalaBasePool::set_nft_attr(&pool_info.basepool, 0, &nft_attr,));
				let nft_attr = PhalaBasePool::get_nft_attr(&pool_info.basepool, 0).unwrap();
		...
		fn test_remove_stake_from_nft() {
			new_test_ext().execute_with(|| {
				...
				let mut nft_attr = PhalaBasePool::get_nft_attr(&pool.basepool, nftid_arr[0]).unwrap();
				assert_eq!(pool.basepool.share_price().unwrap(), 1);
				match PhalaBasePool::remove_stake_from_nft(&mut pool.basepool, 40 * DOLLARS, &mut nft_attr)
				{
		...
		fn test_withdraw() {
			new_test_ext().execute_with(|| {
				...
				let mut item = pool
					...
					.into_iter()
					.find(|x| x.user == 2);
				let nft_attr = PhalaBasePool::get_nft_attr(&pool.basepool, item.unwrap().nft_id).unwrap();
				...
				let user_nft_attr = PhalaBasePool::get_nft_attr(&pool.basepool, nftid_arr[0]).unwrap();
