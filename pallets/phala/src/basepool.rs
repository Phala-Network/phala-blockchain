...
pub mod pallet {
    ...
    use super::{
        extract_dust, is_nondust_balance, BalanceOf,
    };
    ...
    #[pallet::config]
    pub trait Config:
        frame_system::Config + registry::Config + pallet_rmrk_core::Config + mining::Config + Encode + Decode 
        ...
        T: pallet_uniques::Config<CollectionId = CollectionId, ItemId = NftId>,
    {
            ...
            F: FnMut(&BasePool<T::AccountId, BalanceOf<T>>, &mut NftAttr<BalanceOf<T>>, T::AccountId)
        {
            ...
            let shares =
                Self::add_stake_to_new_nft(base_pool, userid.clone(), amount);
            // Lock the funds
        ...
        pub fn add_withdraw_queue(
            ...
            // is called. Or the property of the nft will be overwrote incorrectly.
            let mut nft = Self::get_nft_attr(base_pool, nft_id)?;
            let in_queue_shares = match base_pool
                .withdraw_queue
                .iter()
                .find(|&withdraw| withdraw.user == userid)
            {
                Some(withdraw) => {
                    let withdraw_nft = Self::get_nft_attr(base_pool, withdraw.nft_id)
            ...
            Self::push_withdraw_in_queue(base_pool, &mut nft, nft_id, userid.clone(), shares)?;
            Self::set_nft_attr(base_pool, nft_id, &nft)
                .expect("set nft attr should always success; qed.");
            Self::merge_or_init_nft_for_staker(base_pool, userid)?;
        ...
        pub fn push_withdraw_in_queue(
            ...
            let split_nft_id =
                Self::mint_nft(base_pool, pallet_id(), shares, amount)
        ...
        pub fn mint_nft(
            ...
            let collection_info = pallet_rmrk_core::Collections::<T>::get(base_pool.cid)
                .ok_or(pallet_rmrk_core::Error::<T>::CollectionUnknown)?;
            let nft_id = pallet_rmrk_core::NextNftId::<T>::get(base_pool.cid);
            ...
            Self::set_nft_attr(base_pool, nft_id, &attr)?;
            pallet_rmrk_core::Pallet::<T>::set_lock((base_pool.cid, nft_id), true);
            Ok(nft_id)
        }

        #[frame_support::transactional]
        pub fn burn_nft(base_pool: &BasePool<T::AccountId, BalanceOf<T>>, nft_id: NftId) -> DispatchResult {
            pallet_rmrk_core::Pallet::<T>::set_lock((base_pool.cid, nft_id), false);
        ...
        pub fn merge_or_init_nft_for_staker(
            ...
            let nftid_arr: Vec<NftId> =
                pallet_rmrk_core::Nfts::<T>::iter_key_prefix(base_pool.cid).collect();
            let mut total_stakes: BalanceOf<T> = Zero::zero();
            let mut total_shares: BalanceOf<T> = Zero::zero();
            for nftid in &nftid_arr {
                let nft = pallet_rmrk_core::Nfts::<T>::get(base_pool.cid, nftid)
                ...
                let property = Self::get_nft_attr(base_pool, *nftid)?;
                total_stakes += property.stakes;
                total_shares += property.shares;
                Self::burn_nft(base_pool, *nftid)?;
            }

            Self::mint_nft(base_pool, staker, total_shares, total_stakes)
        }
        ...
        pub fn get_nft_attr(
            ...
            let raw_value: BoundedVec<u8, <T as pallet_uniques::Config>::ValueLimit> =
                pallet_rmrk_core::Pallet::<T>::properties((base_pool.cid, Some(nft_id), key))
