...
use sp_runtime::traits::{
    AtLeast32Bit, CheckedConversion, CheckedSub, Convert, MaybeSerializeDeserialize, Member,
};
...
use xcm::v0::{
    Error as XcmError, ExecuteXcm, Junction, MultiAsset, MultiLocation, NetworkId, Order, Result,
    Xcm,
};
...
impl<T: Config> Module<T> {
    ...
    pub fn do_transfer_to_parachain(
        ...
        match x_currency_id.chain_id {
            ChainId::RelayChain => {
                Self::transfer_relay_chain_tokens_to_parachain(para_id, dest, dest_network, amount)
            }
