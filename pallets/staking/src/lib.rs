...
decl_module! {
	pub struct Module<T: Config> for enum Call where origin: T::Origin {
		...
		pub fn validate(origin, prefs: ValidatorPrefs) {
			let controller = ensure_signed(origin)?;
			ensure!(<pallet_phala::Pallet<T>>::is_controller(controller.clone()), "Not a miner");
...
impl<T: Config> Module<T> {
	...
	fn do_payout_stakers(validator_stash: T::AccountId, era: EraIndex) -> DispatchResultWithPostInfo {
		...
		let current_era = CurrentEra::get().ok_or(
			Error::<T>::InvalidEraToReward.with_weight(<T as Config>::WeightInfo::payout_stakers_alive_staked(0))
		)?;
		let history_depth = Self::history_depth();
		ensure!(
		...
		let controller = Self::bonded(&validator_stash).ok_or(
			Error::<T>::NotStash.with_weight(<T as Config>::WeightInfo::payout_stakers_alive_staked(0))
		)?;
		...
		match ledger.claimed_rewards.binary_search(&era) {
			Ok(_) => Err(
				Error::<T>::AlreadyClaimed.with_weight(<T as Config>::WeightInfo::payout_stakers_alive_staked(0))
			)?,
		...
		Ok(Some(<T as Config>::WeightInfo::payout_stakers_alive_staked(nominator_payout_count)).into())
	}
	...
	for Module<T>
{
	...
	fn voters(
		...
		let weight = <T as Config>::WeightInfo::get_npos_voters(
