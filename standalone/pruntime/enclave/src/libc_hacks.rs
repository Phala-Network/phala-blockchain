use std::{ffi::CStr, fmt::Debug, sync::atomic::AtomicU16};
use sgx_libc::{self as libc, c_void, c_int, size_t, ssize_t, iovec, int32_t, c_char, c_uint};
use sgx_types::{sgx_read_rand, sgx_status_t, sgx_thread_mutex_attr_t, sgx_thread_rwlock_rdlock, sgx_thread_rwlock_t};
...
pub fn print_msg(msg: &str) {
    ...
    let _result = unsafe { write(1, bytes.as_ptr() as _, bytes.len()) };
}
...
pub extern "C" fn writev(fd: c_int, iov: *const iovec, iovcnt: c_int) -> ssize_t {
    unsafe {
        libc::ocall::writev(fd, iov as _, iovcnt)
    }
}
...
pub extern "C" fn _Unwind_GetIPInfo() {
    print_msg("_Unwind_GetIPInfo\n");
    loop {}
}

#[no_mangle]
pub extern "C" fn syscall(num: libc::c_long) -> libc::c_long {
    // if num == 318 { // getrandom
    //     return 0;
    // }
    let numstr = format!("{}", num);
    print_msg("syscall num=");
    print_msg(numstr.as_str());
    print_msg("\n");
    loop {}
}
...
pub extern "C" fn memrchr(s: *mut c_void, c: c_int, n: size_t) -> *mut c_void {
    unsafe {
        libc::memrchr(s as _, c as _, n) as _
    }
}
...
#[no_mangle]
pub extern "C" fn pthread_mutexattr_settype(_attr: *mut libc::pthread_mutexattr_t, typ: c_int) -> c_int {
...
pub extern "C" fn clock_gettime(clk_id: libc::clockid_t, tp: *mut libc::timespec) -> c_int {
    ...
    unsafe {
        libc::ocall::clock_gettime(clk_id, tp);
    }
    0
}
...
pub extern "C" fn open64(_path: *const c_char, _oflag: c_int, _mode: c_int) -> c_int {
    // unsafe {
    //     libc::ocall::open64(path, oflag, mode)
    // }
    panic!("open64 is not supported")
}
...
//     print_msg("t_global_exit_ecall\n");
//     loop {}
// }

// #[no_mangle]
// pub extern "C" fn t_global_init_ecall() {
//     print_msg("t_global_init_ecall\n");
//     loop {}
// }

fn print_syscall(num: c_int) {
