...
var encrypt = (data, key, iv) => {
  ...
  const cipher = (0, import_crypto_browserify2.createCipheriv)(
    ALGO,
    toU8a(key),
    import_buffer.Buffer.from(toU8a(iv))
  );
...
async function create({
  ...
  if (!remotePubkey) {
    ...
    remotePubkey = (0, import_util2.hexAddPrefix)(info.publicKey);
  }
  ...
  let gasPrice = new import_util2.BN(0);
  if (autoDeposit) {
  ...
  const txContracts = (dest, value, gas, storageDepositLimit, encParams) => {
    let deposit = new import_util2.BN(0);
    if (autoDeposit) {
      const gasFee = new import_util2.BN(gas.refTime).mul(gasPrice);
      deposit = new import_util2.BN(value).add(gasFee).add(new import_util2.BN(storageDepositLimit || 0));
    }
    return command({
